...
import { IoChatboxEllipses, IoChatboxEllipsesOutline ,IoAddCircle , IoAddCircleOutline ,IoNotifications, IoNotificationsOutline, IoSearchOutline , IoSearchSharp ,IoHome, IoHomeOutline, IoLogOut, } from "react-icons/io5";
...
function NavBar() {
  ...
  return (
    <div className='bg-black border border-gray-600 absolute left-0 bottom-0 w-screen h-16 md:top-0 md:h-screen md:w-2/12 flex min-w-fit border-l-0
    md:flex-col justify-center place-items-center md:justify-start md:place-items-start md:gap-2 p-2 text-xl'>
          ...
          
          {
            navRouter.map((navItem)=>{
              return(
                <NavLink to={navItem.path} className={navLinkClassName}>
                {
                  ({isActive})=>{
                    if(isActive){
